...
const ContactsPage = () => {
  ...
  const contactItems = [
    ...
    { icon: 'fa-envelope', title: 'Email', value: '8888888gorni@gmail.com', detail: 'Ответ в течение 24 часов' },
  ];
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-gray-900/80 via-gray-800/80 to-black border border-white/10 mb-10">
          ...
          <div className="relative py-10 px-8 text-center">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <span className="text-gray-400 text-[10px] tracking-[0.3em]">СВЯЗЬ</span>
              <div className="w-8 h-0.5 bg-white/40"></div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:border-white/30 focus:outline-none transition duration-300"
                  required
                />
              </div>
              <div>
              ...
              <div>
                <label className="text-white/40 text-[10px] font-bold block mb-1 tracking-wider">СООБЩЕНИЕ *</label>
                <textarea
                  value={formData.message}
                  onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                  rows={5}
                  className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:border-white/30 focus:outline-none transition duration-300 resize-none"
                  required
                />
              </div>
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-white text-black py-3.5 rounded-xl font-black text-sm tracking-wider hover:bg-white/90 transition-all duration-300 disabled:opacity-50"
              >
