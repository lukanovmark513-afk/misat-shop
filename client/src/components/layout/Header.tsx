      ...
      <header className={`fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a] py-3 transition-all duration-300 ${isScrolled ? 'backdrop-blur-xl bg-[#0a0a0a]/95' : ''}`}>
        <div className="w-full px-4 md:px-8 lg:px-16">
          <div className="flex justify-between items-center">
            ...
            {/* Desktop Navigation - с подсветкой активной вкладки */}
            <nav className="hidden md:flex items-center gap-8">
              <Link
                to="/"
                className={`relative text-sm uppercase tracking-wider transition-all duration-300 pb-1 ${
                  isActive('/')
                    ? 'text-white'
              ...
              >
                Главная
                {isActive('/') && (
                  <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-white rounded-full animate-slide-in" />
                )}
              </Link>
